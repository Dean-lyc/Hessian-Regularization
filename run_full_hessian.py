...
for epoch in range(epochs):
    ...
    for times, data in tqdm(enumerate(trainLoader, 0)):
        ...
        for param in params:
            hessian_kk = hessian(loss_super, param)
        ...
        #         # print(grads[i][j].size(),grads[i][j])
        #         hessian_tr += torch.autograd.grad(grads[i][j], params[i][j], grad_outputs = params[i][j], only_inputs=True, retain_graph=True)
        # for i in grads:
